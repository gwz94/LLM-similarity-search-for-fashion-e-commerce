...
def test_search_products_success(mock_db, mock_embedding, mock_reranker):
    ...
    mock_db.search_products.side_effect = [MOCK_IN_STOCK_RESULTS, MOCK_OUT_OF_STOCK_RESULTS]
...
def test_search_products_invalid_query():
    ...
    response = client.post("/search", json={"query": "test<script>alert('xss')</script>"})
